...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, spike_type='sum', \
        ...
        self.act = HoyerBiAct(num_features=inplanes, spike_type=spike_type, x_thr_scale=x_thr_scale, if_spike=if_spike, if_set_0=if_set_0)
...
class BasicBlock_double(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, spike_type='sum', \
        ...
        self.bi_act1        = HoyerBiAct(num_features=inplanes, spike_type=spike_type, x_thr_scale=x_thr_scale, if_spike=if_spike, if_set_0=if_set_0)
        self.bi_conv1       = conv3x3(inplanes, planes,stride=stride)
        self.bn1            = nn.BatchNorm2d(planes)
        self.bi_act2        = HoyerBiAct(num_features=inplanes, spike_type=spike_type, x_thr_scale=x_thr_scale, if_spike=if_spike, if_set_0=if_set_0)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, spike_type='sum', x_thr_scale=1.0, if_spike=True, if_set_0=False):
        ...
        self.act3 = HoyerBiAct(num_features=planes, spike_type=spike_type, x_thr_scale=x_thr_scale, if_spike=if_spike, if_set_0=if_set_0)
        ...
        if stride != 1 or inplanes != self.expansion*planes:
            ...
            self.downsample = nn.Sequential(
                ...
                HoyerBiAct(num_features=inplanes, spike_type=spike_type, x_thr_scale=x_thr_scale, if_spike=if_spike, if_set_0=if_set_0),
    ...
    def __init__(self, block, num_blocks, labels=10, dataset = 'CIFAR10', kernel_size=3, linear_dropout=0.1, conv_dropout=0.1, default_threshold=1.0, \
        ...
        if dataset == 'CIFAR10':
            ...
            self.conv1 = nn.Sequential(
                                ...
                                nn.BatchNorm2d(64),
                                HoyerBiAct(num_features=64, spike_type=self.spike_type, x_thr_scale=self.x_thr_scale, if_spike=self.if_spike, if_set_0=self.if_set_0),
                                ...
                                # nn.MaxPool2d(2),
                                # nn.BatchNorm2d(64),
                                # HoyerBiAct(num_features=64, spike_type=self.spike_type, if_set_0=self.if_set_0)
                                )
        ...
        # self.fc_act = HoyerBiAct(spike_type='sum', x_thr_scale=self.x_thr_scale, if_spike=self.if_spike, if_set_0=self.if_set_0)
        # self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        # self.fc = nn.Linear(512 * block.expansion, labels)
# ------------------ dynamic layers supporting 3- or 4-stage configs ------------------
        if len(num_blocks) not in (3, 4):
            raise ValueError("num_blocks must be length 3 (CIFAR-style) or 4 (ImageNet-style)")

        # determine plane sizes used per stage
        planes_seq = [64, 128, 256, 512] if len(num_blocks) == 4 else [64, 128, 256]

        self.layers = nn.ModuleList()
        for idx, nblocks in enumerate(num_blocks):
            planes = planes_seq[idx]
            # stage 0: stride=1 (no downsample), subsequent stages: stride=2
            stride = 1 if idx == 0 else 2
            layer = self._make_layer(block, planes, nblocks, stride=stride)
            self.layers.append(layer)

        # final classifier setup: final channels depend on whether we have 3 or 4 stages
        final_planes = planes_seq[-1] * block.expansion
        self.fc_act = HoyerBiAct(spike_type='sum', x_thr_scale=self.x_thr_scale,
                                if_spike=self.if_spike, if_set_0=self.if_set_0)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(final_planes, labels)
# -----------------------------------------------------------------------------------
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            downsample = nn.Sequential(
                ...
                HoyerBiAct(num_features=self.inplanes, spike_type=self.spike_type, x_thr_scale=self.x_thr_scale, if_spike=self.if_spike, if_set_0=self.if_set_0),
            ...
            # downsample = nn.Sequential(
            #     HoyerBiAct(num_features=self.inplanes, spike_type=self.spike_type, x_thr_scale=self.x_thr_scale, if_set_0=self.if_set_0),
    ...
    def forward(self, x):
        ...
        for layers in self.layers:
            for l in layers:
                # print(i, x.shape)
                # act_out += self.hoyer_loss(x.clone())
                # act_out += self.hoyer_loss(x.clone(), thr=l.act.threshold.clone().detach())
                x = l(x)
            # x = layers(x)
            # act_out += self.hoyer_loss(x.clone())
        # x = self.bn1(x) # for 2.0
        ...
        x = x.view(x.size(0), -1)
        # act_out += self.hoyer_loss(x.clone())
        # act_out += self.hoyer_loss(x.clone(), thr=self.fc_act.threshold.clone().detach())
        x = self.fc_act(x)
        x = self.fc(x)
...
def resnet20(pretrained=False, **kwargs):
    ...
    model = HoyerResNet(BasicBlock_double, [3, 3, 3], **kwargs)
    print("[DEBUG]: resnet20")
