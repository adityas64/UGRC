...
from quant.quantization import *
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.residual_function = nn.Sequential(
            quan_Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=False),
            nn.BatchNorm2d(out_channels),
            IF(),
            quan_Conv2d(out_channels, out_channels * BasicBlock.expansion, kernel_size=3, padding=1, bias=False),
        ...
        if stride != 1 or in_channels != BasicBlock.expansion * out_channels:
            self.shortcut = nn.Sequential(
                quan_Conv2d(in_channels, out_channels * BasicBlock.expansion, kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(out_channels * BasicBlock.expansion)
            )
        self.act = IF()
...
class ResNet4Cifar(nn.Module):
    def __init__(self, block, num_block, num_classes=10):
        ...
        self.expand = ExpandTemporalDim(0)
        self.conv1 = nn.Sequential(
            quan_Conv2d(3, 16, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(16),
            IF())
        # we use a different inputsize than the original paper
        # so conv2_x's stride is 1
