...
class QuantConv2d(nn.Conv2d):
    """Conv2d layer with 8-bit weight quantization via STE"""
    def forward(self, input):
        W_q = self.weight.data
        max_val = W_q.abs().max()
        scale = max_val / 127 if max_val != 0 else 1.0
        W_q = torch.clamp((W_q / scale).round(), -128, 127) * scale
        return nn.functional.conv2d(input, W_q, self.bias, self.stride,
                                    self.padding, self.dilation, self.groups)

class QuantLinear(nn.Linear):
    """Linear layer with 1-bit weight quantization via STE"""
    def forward(self, input):
        W_q = self.weight.data.sign()  # -1/+1
        return nn.functional.linear(input, W_q, self.bias)
...
class VGG16_light_only_bn(nn.Module):
    def __init__(self, vgg_name='VGG16', labels=1000, dataset = 'IMAGENET', kernel_size=3, linear_dropout=0.1, conv_dropout=0.1, default_threshold=1.0, \
        ...
        if dataset=='IMAGENET':
            self.classifier = nn.Sequential(
                            QuantLinear((im_size//32)**2*512, 4096, bias=False),
                            HoyerBiAct(spike_type=fc_spike_type, x_thr_scale=self.x_thr_scale, if_spike=self.if_spike, if_set_0=self.if_set_0),
                            nn.Dropout(linear_dropout),
                            QuantLinear(4096, 4096, bias=False),
                            HoyerBiAct(spike_type=fc_spike_type, x_thr_scale=self.x_thr_scale, if_spike=self.if_spike, if_set_0=self.if_set_0),
                            nn.Dropout(linear_dropout),
                            QuantLinear(4096, labels, bias=False)
            )
        elif dataset=='CIFAR10':
            self.classifier = nn.Sequential(
                            QuantLinear(2048, 4096, bias=False),
                            HoyerBiAct(spike_type=fc_spike_type, x_thr_scale=self.x_thr_scale, if_spike=self.if_spike, if_set_0=self.if_set_0),
                            nn.Dropout(linear_dropout),
                            QuantLinear(4096, 4096, bias=False),
                            HoyerBiAct(spike_type=fc_spike_type, x_thr_scale=self.x_thr_scale, if_spike=self.if_spike, if_set_0=self.if_set_0),
    ...
    def _make_layers(self, cfg):
        ...
        for i,x in enumerate(cfg):
            ...
            if x == 'M':
                continue
            conv = QuantConv2d(in_channels, x, kernel_size=3, padding=1, stride=1, bias=False)
    ...
    def _initialize_weights_qat(self):
        for m in self.modules():
            if isinstance(m, QuantConv2d):
                # He initialization for conv layers (8-bit quantization)
                ...
                # 1-bit FC layers: initialize uniformly to [-1,1] for better forward signal
